...
class Gripper1(Sofa.Core.Controller, PivotizedArticulatedInstrument):
    def __init__(
        self,
        ...
        #print('col sphere pos',collision_spheres_config["positions"])
        ...
        self.intestine = intestine_to_grasp
        ...
        self.num_spheres = 12#len(collision_spheres_config["positions"])

        # Add sphere collision models to the gripper jaw amd the static gripper jaw
        self.collision_node_shaft = self.physical_shaft_node.addChild("approximated_collision_shaft_jaw_1")
        ...
        self.collision_mechanical_object = {
            "shaft_1": self.collision_node_shaft.addObject("MechanicalObject", template="Vec3d", position=[[0, 0, 5 + i * 2] for i in range(12)]),
            "jaw_1": self.collision_node_jaw.addObject("MechanicalObject", template="Vec3d", position=[[0, 0, 5 + i * 2] for i in range(12)]),
        }
        ...
        self.contact_listener = {
            "shaft_1": self.gripper_node.addObject(
                "ContactListener",
                name="contact_listener_intestine_shaft_1",
                collisionModel1=self.intestine.deformable_object.collision_model_node.TriangleCollisionModel.getLinkPath(),
                collisionModel2=self.sphere_collisions_shaft.getLinkPath(),
            ),
            "jaw_1": self.gripper_node.addObject(
                "ContactListener",
                name="contact_listener_intestine_jaw_1",
                collisionModel1=self.intestine.deformable_object.collision_model_node.TriangleCollisionModel.getLinkPath(),
        ...
        self.grasping_force_field = {
            "shaft_1": self.intestine.deformable_object.collision_model_node.addObject(
                "RestShapeSpringsForceField",
                name="grasping_force_field_gripper_shaft_1",
                external_rest_shape=self.collision_mechanical_object["shaft_1"].getLinkPath(),
                drawSpring=True,
            ...
            "jaw_1": self.intestine.deformable_object.collision_model_node.addObject(
                "RestShapeSpringsForceField",
                name="grasping_force_field_gripper_jaw_1",
        ...
        #print(self.collision_mechanical_object["jaw_1"].getLinkPath())
        self.collision_deactivated_steps = collision_deactivated_steps
    ...
    def identify_execessive_force(self):
        if self.max_grasp_force is not None:
            for jaw in self.contact_listener:
                ...
                jaw_forces = self.collision_mechanical_object["jaw_1"].force.array()[active_grasps]
                intestine_forces = self.intestine.deformable_object.collision_model_node.MechanicalObject.force.array()[indices[active_grasps]]
                excessive_jaw_indices = np.argwhere(np.abs(jaw_forces) > self.max_grasp_force)
                excessive_intestine_indices = np.argwhere(np.abs(intestine_forces) > self.max_grasp_force)
                if excessive_jaw_indices.any() or excessive_intestine_indices.any():
                    [print(f"intestine: {intestine_forces[tuple(index)]}") for index in excessive_intestine_indices]
                    [print(f"Jaw_1: {jaw_forces[tuple(index)]}") for index in excessive_jaw_indices]
    ...
    def _attempt_grasp(self):
        ...
        for jaw, contact_listener in self.contact_listener.items():
            ...
            for id, contact in enumerate(contacts):
                triangle_index_on_intestine = contact[1] if contact[0] == 0 else contact[3]
                sphere_index_on_jaw = contact[3] if contact[0] == 0 else contact[1]

                triangle_point_index = self.intestine.deformable_object.collision_model_node.loader.triangles.array()[triangle_index_on_intestine][0]

                intestine_position = self.intestine.deformable_object.collision_model_node.MechanicalObject.position.array()[triangle_point_index]

                sphere_position = np.array(list(contact_points[id][1]))

                distance = np.linalg.norm(intestine_position - sphere_position)
        ...
        if found_contacts_on_both_jaws:
            for num, jaw in enumerate(self.contact_listener):
                with self.grasping_force_field[jaw].points.writeable() as indices, self.grasping_force_field[jaw].stiffness.writeable() as stiffness, self.grasping_force_field[jaw].angularStiffness.writeable() as angular_stiffness:
                    indices[:] = mapping[num][:, 0]  # intestine indices, mapped to the sphere indices
    ...
    def get_grasp_center_position(self) -> np.ndarray:
        ...
        return np.mean((self.collision_mechanical_object["shaft_1"].position.array(), self.collision_mechanical_object["jaw_1"].position.array()), axis=(0, 1))
