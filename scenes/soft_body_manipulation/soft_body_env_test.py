...
from .feature_extraction.cae import get_latent_vector
from .feature_extraction import VGG16
from .feature_extraction.transparency import overlay_images
from.feature_extraction.flare import flare
...
# latent_vec_size = 32768 # CAE
latent_vec_size =  4608 # VGG16

# load vgg16
vgg16 = VGG16.load_model()
...
class SoftBodyManipulationEnv(SofaEnv):
    ...
    def __init__(
        ...
        if self.observation_type == ObservationType.STATE:
            # self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(latent_vec_size,), dtype=np.float32)
            self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(6, 2048), dtype=np.float32)
        elif self.observation_type == ObservationType.RGB:
            self.observation_space = spaces.Box(low=0, high=255, shape=image_shape + (3*self.n,), dtype=np.uint8)
    ...
    def _get_observation(self, image_observation: Union[np.ndarray, None]) -> Union[np.ndarray, dict]:
        ...
        if len(self.frames) == 0 :
            for i in range(4):
                self.frames.append(Image.fromarray(image_observation.copy()))
        else:
            self.frames.pop(0)
            self.frames.append(Image.fromarray(image_observation.copy()))
        ...
        if self.observation_type == ObservationType.RGB:
            ...
            # return np.concatenate(np.array(self.frames), axis=2)
            return overlay_images(self.frames)
            ...
            return flare(copy.copy(self.frames))
            # return VGG16.get_latent_vector(vgg16, image_observation)
            # return get_latent_vector(image_observation)
    ...
    def _get_reward(self, image: np.ndarray) -> float:
        ...
        if self.active_phase == Phase.TOUCH:
            ...
            try:
                ...
                cv2.imwrite("/home/saketh/SOFA/v23.06.00/bin/lapgym/sofa_env/sofa_env/scenes/soft_body_manipulation/error_image.png",image)
                reward_features["img_to_img"] = previous_reward_features["img_to_img"]
                avg_distance = -1*previous_reward_features["img_to_img"]
            '''
            kp2, desc2 = self.brief.compute(image, self.star.detect(image, None))
            matches = self.bf.match(self.desc1, desc2)
            avg_distance = sum(m.distance for m in matches) / len(matches) if matches else 1000
        ...
        if self.active_phase == Phase.TOUCH and avg_distance<15.0:
