...
class SoftBodyManipulationEnv(SofaEnv):
    ...
    def __init__(
        ...
        reward_amount_dict: dict =  {
            ...
            Phase.GRASP: {
                "distance_gripper1_graspable_region": -1.0,
                "delta_distance_gripper1_graspable_region": -10.0,
            },
            Phase.TOUCH: {
                "img_to_img":-10.0,
            },
        ...
        self.goal_img = cv2.resize(cv2.imread('/home/saketh/SOFA/v23.06.00/bin/lapgym/sofa_env/sofa_env/scenes/soft_body_manipulation/exit_image_1.png'), (512, 512))
        ...
        if individual_agents:
            ...
            if action_type == ActionType.CONTINUOUS:
                self.action_space = gym.spaces.Dict(
                    {
                        "gripper1": gym.spaces.Box(low=-1.0, high=1.0, shape=(action_dimensionality,), dtype=np.float32),
                    }
                )
                ...
                self.action_space = gym.spaces.Dict(
                    {
                        "gripper1": gym.spaces.Discrete(action_dimensionality * 2 + 1)
                    }
                )
    ...
    def _init_sim(self) -> None:
        ...
        self.intestine_surface_mechanical_object = self.intestine.collision_model_node.MechanicalObject
    ...
    def reset(self) -> Union[np.ndarray, None]:
        ...
        if self.active_phase >= Phase.TOUCH:#change_
            ...
            while not self.gripper1.grasp_established:
                # Move the gripper1 to the intestine.
                optimal_state = np.array([3.0, 20.0, 95.0, 215.0, 30.0])
                if self.gripper1.randomize_starting_position and not skip_random_grasping:
                    optimal_state += self.rng.uniform(-3.0, 3.0, optimal_state.shape)
                action_plan = create_linear_motion_action_plan(optimal_state, self.gripper1.get_ptsda_state(), self.time_step, self._maximum_state_velocity)
                for action in action_plan:
                    self.gripper1.do_action(action)
                    self.sofa_simulation.animate(self._sofa_root_node, self._sofa_root_node.getDt())
                    if self.render_mode == RenderMode.HUMAN:
                        self._maybe_update_rgb_buffer()

                # Close the gripper1 jaws
                optimal_state[-1] = 5.0
                action_plan = create_linear_motion_action_plan(optimal_state, self.gripper1.get_ptsda_state(), self.time_step, self._maximum_state_velocity)
                for action in action_plan:
                    self.gripper1.do_action(action)
                ...
                # Animate several timesteps without actions to let the gripper1 establish a new grasp.
                for _ in range(10):
                    self.sofa_simulation.animate(self._sofa_root_node, self._sofa_root_node.getDt())
                    if self.render_mode == RenderMode.HUMAN:
                        self._maybe_update_rgb_buffer()
                ...
                if grasping_attempts > 10 and not skip_random_grasping:
                    ...
                    if self.limit_grasping_attempts_in_reset:
                        raise RuntimeError(f"Could not grasp the intestine on reset to phase {self.active_phase}.")
                    else:
                        print("[Warning] Grasping in reset unsuccessful...trying again with new reset.")
                        self.reset()
            '''
            if self.active_phase >= Phase.TOUCH:
                # With the grasped intestine, move the gripper1 up, to expose the visual target.
                target_visible = False

                # If we do not render the scene, figuring out if the target is visible, is not possible.
                # In this case, we just pull outh the gripper1 very far, to make sure the target is exposed.
                ...
                while not target_visible:
                    ...
                    if self.gripper1.get_ptsda_state()[3] != optimal_state[3]:
                        print("[WARNING] gripper1 hit state or workspace limits in reset without visualizing the target. Will reset again.")
                        self.reset()
        ...
        grasp_region_center = np.mean(self.intestine.node.MechanicalObject.position.array()[self.intestine_indices_in_grasping_roi], axis=0)
        ...
        print(np.linalg.norm(self.gripper1.get_grasp_center_position() - grasp_region_center))
    ...
    def _get_reward(self, image: np.ndarray) -> float:
        ...
        if not reward_features["intestine_is_grasped"]:
        ...
        if self.active_phase == Phase.TOUCH:
            ...
            reward_features["img_to_img"] = -1*(sum(m.distance for m in matches) / len(matches) if matches else 1000)
            #print(reward_features["img_to_img"])
        if self.active_phase == Phase.TOUCH and reward_features["img_to_img"]>-15.0:
        ...
        l2 = self.reward_amount_dict[Phase.ANY]
        for key in l1:
            if key in l2:
    ...
    def _get_reward_features(self, image: np.ndarray, previous_reward_features: dict) -> dict:
        ...
        kp2, desc2 = self.brief.compute(image, self.star.detect(image, None))
        matches = self.bf.match(self.desc1, desc2)
        reward_features["img_to_img"] = -1*(sum(m.distance for m in matches) / len(matches) if matches else 1000)*self.gripper1.grasp_established
        #print(reward_features["img_to_img"])
